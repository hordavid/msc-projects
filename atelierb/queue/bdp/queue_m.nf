...
THEORY ListVariablesX IS
  ...
  Local_List_Variables(Machine(queue_m))==(is_empty,xx);
  List_Variables(Machine(queue_m))==(is_empty,xx);
  External_List_Variables(Machine(queue_m))==(is_empty,xx)
END
&
...
THEORY ListInvariantX IS
  ...
  List_Invariant(Machine(queue_m))==(xx: 0..100 +-> 0..100 & is_empty: BOOL & xx: FIN(xx) & is_empty = TRUE <=> (card(xx) = 0) & is_empty = FALSE <=> card(xx)>0 & !(ii,jj).(card(xx)>1 & ii: dom(xx) & jj: dom(xx) & ii>jj => xx(ii)>xx(jj)))
END
&
...
THEORY ListInitialisationX IS
  Expanded_List_Initialisation(Machine(queue_m))==(xx,is_empty:={},TRUE);
  Context_List_Initialisation(Machine(queue_m))==(skip);
  List_Initialisation(Machine(queue_m))==(xx:={} || is_empty:=TRUE)
END
&
...
THEORY ListSubstitutionX IS
  Expanded_List_Substitution(Machine(queue_m),is_full_op)==(btrue | is_empty = TRUE ==> ret:=FALSE [] not(is_empty = TRUE) ==> ret:=TRUE);
  Expanded_List_Substitution(Machine(queue_m),is_empty_op)==(btrue | ret:=is_empty);
  Expanded_List_Substitution(Machine(queue_m),get_min)==(card(xx)>0 & dom(xx) = 0..card(xx) | card(xx) = 1 ==> ret,is_empty,xx:=xx(0),TRUE,{} [] not(card(xx) = 1) ==> @(t_xx,t_is_empty,t_min).(t_xx: 0..100 +-> 0..100 & t_min: 0..100 & t_is_empty: BOOL & dom(t_xx) = 0..100 & dom(xx) = 0..100 & t_xx: FIN(t_xx) & t_min = xx(card(xx)) & !(jj,kk).(jj: dom(t_xx) & kk: dom(t_xx) & jj>kk => t_xx(jj)>t_xx(kk)) & t_is_empty = TRUE <=> (card(t_xx) = 0) & t_is_empty = FALSE <=> card(t_xx)>0 ==> xx,ret,is_empty:=t_xx,t_min,t_is_empty));
  Expanded_List_Substitution(Machine(queue_m),empty)==(card(xx): 1..100 | xx,is_empty:={},TRUE);
  Expanded_List_Substitution(Machine(queue_m),add)==(item: 0..100 & dom(xx) = 0..card(xx) | @(t_xx,t_is_empty).(t_xx: 0..100 +-> 0..100 & t_is_empty: BOOL & t_xx: FIN(t_xx) & ran(t_xx) = ran(xx)\/{item} & card(t_xx): 1..card(xx)+1 & card(t_xx)>0 <=> !(ii,jj).(ii: dom(t_xx) & jj: dom(t_xx) & ii>jj => t_xx(ii)>t_xx(jj)) & t_is_empty = TRUE <=> (card(t_xx) = 0) & t_is_empty = FALSE <=> card(t_xx)>0 ==> xx,is_empty:=t_xx,t_is_empty));
  List_Substitution(Machine(queue_m),add)==(ANY t_xx,t_is_empty WHERE t_xx: 0..100 +-> 0..100 & t_is_empty: BOOL & t_xx: FIN(t_xx) & ran(t_xx) = ran(xx)\/{item} & card(t_xx): 1..card(xx)+1 & card(t_xx)>0 <=> !(ii,jj).(ii: dom(t_xx) & jj: dom(t_xx) & ii>jj => t_xx(ii)>t_xx(jj)) & t_is_empty = TRUE <=> (card(t_xx) = 0) & t_is_empty = FALSE <=> card(t_xx)>0 THEN xx:=t_xx || is_empty:=t_is_empty END);
  List_Substitution(Machine(queue_m),empty)==(xx:={} || is_empty:=TRUE);
  List_Substitution(Machine(queue_m),get_min)==(IF card(xx) = 1 THEN ret:=xx(0) || is_empty:=TRUE || xx:={} ELSE ANY t_xx,t_is_empty,t_min WHERE t_xx: 0..100 +-> 0..100 & t_min: 0..100 & t_is_empty: BOOL & dom(t_xx) = 0..100 & dom(xx) = 0..100 & t_xx: FIN(t_xx) & t_min = xx(card(xx)) & !(jj,kk).(jj: dom(t_xx) & kk: dom(t_xx) & jj>kk => t_xx(jj)>t_xx(kk)) & t_is_empty = TRUE <=> (card(t_xx) = 0) & t_is_empty = FALSE <=> card(t_xx)>0 THEN xx:=t_xx || ret:=t_min || is_empty:=t_is_empty END END);
  List_Substitution(Machine(queue_m),is_empty_op)==(ret:=is_empty);
  List_Substitution(Machine(queue_m),is_full_op)==(IF is_empty = TRUE THEN ret:=FALSE ELSE ret:=TRUE END)
END
&
...
THEORY ListANYVarX IS
  List_ANY_Var(Machine(queue_m),add)==((Var(t_xx) == SetOf(btype(INTEGER,?,?)*btype(INTEGER,?,?))),(Var(t_is_empty) == btype(BOOL,?,?)));
  List_ANY_Var(Machine(queue_m),empty)==(?);
  List_ANY_Var(Machine(queue_m),get_min)==((Var(t_xx) == SetOf(btype(INTEGER,?,?)*btype(INTEGER,?,?))),(Var(t_is_empty) == btype(BOOL,?,?)),(Var(t_min) == btype(INTEGER,?,?)));
  List_ANY_Var(Machine(queue_m),is_empty_op)==(?);
  List_ANY_Var(Machine(queue_m),is_full_op)==(?)
END
&
THEORY ListOfIdsX IS
  List_Of_Ids(Machine(queue_m)) == (? | ? | is_empty,xx | ? | add,empty,get_min,is_empty_op,is_full_op | ? | ? | ? | queue_m);
...
THEORY VariablesEnvX IS
  Variables(Machine(queue_m)) == (Type(is_empty) == Mvl(btype(BOOL,?,?));Type(xx) == Mvl(SetOf(btype(INTEGER,?,?)*btype(INTEGER,?,?))))
END
&
